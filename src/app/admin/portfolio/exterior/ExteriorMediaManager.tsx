...
import { createPortfolioExterior, updatePortfolioExterior, deletePortfolioExterior } from "../../actions";
    ...
    return (
      <form
        ...
        action={async (fd: FormData) => {
          setSaving(true);
          setMsg(null);
          const res = await updatePortfolioExterior(row.id, {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded border border-zinc-700 bg-zinc-800/50 p-3">
      ...
      <div className="flex gap-2">
        ...
        <form action={async () => {
          if (confirm("Obrisi ovu sliku?")) { await deletePortfolioExterior(row.id); window.location.reload(); }
        }}>
  ...
  return (
    <form
      ...
      action={async (fd: FormData) => {
        setSaving(true);
        setMsg(null);
        const res = await createPortfolioExterior({
          image_url: (fd.get("image_url") as string) || "",
...
export function ExteriorMediaManager({ rows }: { rows: Record<string, unknown>[] }) {
