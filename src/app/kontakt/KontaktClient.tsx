...
import { ChevronDown, CheckCircle2, AlertCircle, Instagram, Mail, Phone } from 'lucide-react';
import type { KontaktPageContent } from "@/lib/supabase-content";
...
type Props = {
  content: KontaktPageContent | null;
};
...
export default function KontaktClient({ content }: Props) {
  ...
  // Fallback vrednosti
  const pageEyebrow = k?.pageEyebrow || "Kontakt";
  const pageTitle = k?.pageTitle || "Stupimo u kontakt";
  const pageSubtitle = k?.pageSubtitle || "Pratite naš rad na društvenim mrežama ili nas kontaktirajte direktno.";
  const phoneLabel = k?.phoneLabel || "Telefon";
  const phoneValue = k?.phoneValue || "+381 62 923 3484";
  const phoneHref = phoneValue ? `tel:${phoneValue.replace(/\s/g, "")}` : "#";
  const instagramLabel = k?.instagramLabel || "Instagram";
  const instagramValue = k?.instagramValue || "https://www.instagram.com/luminus_photography/";
  const emailLabel = k?.emailLabel || "Email";
  const emailValue = k?.emailValue || "info@luminus.rs";
  const tiktokLabel = k?.tiktokLabel || "TikTok";
  const tiktokValue = k?.tiktokValue || "https://www.tiktok.com/@odgajivacnicaroyal?lang=en";
  const bookingEyebrow = k?.bookingEyebrow || "Rezervacija";
  const bookingTitle = k?.bookingTitle || "Zakažite\nTermin.";
  const bookingIntro = k?.bookingIntro || "Spremni ste da istaknete vašu nekretninu? Popunite formu i odaberite željeni termin.";
  const successTitle = k?.successTitle || "Zahtev Poslat";
  const successMessage = k?.successMessage || "Hvala vam na interesovanju. Kontaktiraćemo vas uskoro radi potvrde termina.";
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', city: '', package: '', date: '', time: '', message: ''
  });
  ...
  const validate = () => {
    ...
    if (formData.time) {
      ...
      if (hour < 8 || hour >= 18) newErrors.time = 'Radno vreme je od 08:00 do 18:00';
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!FORMSPREE_ENDPOINT) {
      setSubmitError(`Forma nije podešena. Kontaktirajte nas direktno: ${emailValue} ili ${phoneValue}.`);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setSubmitError(`Greška pri slanju. Kontaktirajte nas direktno: ${emailValue}`);
        return;
      }
      setIsSubmitted(true);
    } catch {
      setSubmitError(`Slanje nije uspelo. Pošaljite nam email na ${emailValue} ili nazovite ${phoneValue}.`);
    } finally {
    ...
    <div className="bg-black min-h-screen text-white pt-32 pb-24">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6">
          {/* Phone */}
          <motion.a href={phoneHref} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            ...
            <div>
              <h3 className="text-lg md:text-xl font-serif mb-1 md:mb-2">{phoneLabel}</h3>
              <p className="text-white/50 text-xs md:text-sm">{phoneValue}</p>
            </div>
          </motion.a>

          {/* Instagram */}
          <motion.a href={instagramValue} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }}
            className="bg-white/5 border border-white/10 p-6 md:p-8 flex flex-row md:flex-col items-center text-left md:text-center hover:bg-white/10 hover:border-[#D4AF37] transition-all duration-300 group">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-black border border-white/20 flex items-center justify-center mr-4 md:mr-0 md:mb-6 group-hover:border-[#D4AF37] transition-colors flex-shrink-0">
              <Instagram className="w-5 h-5 md:w-6 md:h-6 text-white group-hover:text-[#D4AF37] transition-colors" />
            </div>
            <div>
              <h3 className="text-lg md:text-xl font-serif mb-1 md:mb-2">{instagramLabel}</h3>
              <p className="text-white/50 text-xs md:text-sm break-all">{instagramValue}</p>
            </div>
          </motion.a>

          {/* Email */}
          <motion.a href={`mailto:${emailValue}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
            className="bg-white/5 border border-white/10 p-6 md:p-8 flex flex-row md:flex-col items-center text-left md:text-center hover:bg-white/10 hover:border-[#D4AF37] transition-all duration-300 group">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-black border border-white/20 flex items-center justify-center mr-4 md:mr-0 md:mb-6 group-hover:border-[#D4AF37] transition-colors flex-shrink-0">
              <Mail className="w-5 h-5 md:w-6 md:h-6 text-white group-hover:text-[#D4AF37] transition-colors" />
            </div>
            <div>
              <h3 className="text-lg md:text-xl font-serif mb-1 md:mb-2">{emailLabel}</h3>
              <p className="text-white/50 text-xs md:text-sm">{emailValue}</p>
            </div>
          </motion.a>

          {/* TikTok */}
          <motion.a href={tiktokValue} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}
            className="bg-white/5 border border-white/10 p-6 md:p-8 flex flex-row md:flex-col items-center text-left md:text-center hover:bg-white/10 hover:border-[#D4AF37] transition-all duration-300 group">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-black border border-white/20 flex items-center justify-center mr-4 md:mr-0 md:mb-6 group-hover:border-[#D4AF37] transition-colors flex-shrink-0">
              <TikTokIcon className="w-5 h-5 md:w-6 md:h-6 text-white group-hover:text-[#D4AF37] transition-colors" />
            </div>
            <div>
              <h3 className="text-lg md:text-xl font-serif mb-1 md:mb-2">{tiktokLabel}</h3>
              <p className="text-white/50 text-xs md:text-sm break-all">{tiktokValue}</p>
            </div>
          </motion.a>
        </div>
      </div>
      ...
      <div id="zakazi" className="max-w-7xl mx-auto px-6 pt-12 md:pt-20 border-t border-white/10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
            ...
            <p className="text-[#D4AF37] uppercase tracking-[0.2em] text-[10px] md:text-xs font-semibold mb-2 md:mb-4">{bookingEyebrow}</p>
            <h2 className="text-4xl md:text-7xl font-serif mb-4 md:mb-8 leading-tight whitespace-pre-line">{bookingTitle}</h2>
            <p className="text-white/60 text-base md:text-lg leading-relaxed mb-8 md:mb-12 max-w-md">{bookingIntro}</p>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }}
            ...
            {isSubmitted ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-12 md:py-20">
                ...
                <h3 className="text-2xl md:text-3xl font-serif mb-4">{successTitle}</h3>
                <p className="text-white/60 text-sm md:text-base">{successMessage}</p>
                <button onClick={() => setIsSubmitted(false)} className="mt-8 md:mt-12 text-[#D4AF37] text-xs uppercase tracking-widest hover:text-white transition-colors">
                  Pošalji novi zahtev
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div className="space-y-1 md:space-y-2">
                    ...
                    <input type="date" name="date" value={formData.date} onChange={handleChange} min={minDate}
                    ...
                    <div className="relative">
                      <select name="time" value={formData.time} onChange={handleChange}
                        ...
                        {Array.from({ length: 20 }, (_, i) => {
                          ...
                          return <option key={val} value={val} className="bg-black">{val}</option>;
                        })}
                      </select>
                      <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-white/50 pointer-events-none" size={16} />
                    </div>
                ...
                {submitError && (
                  <div className="mt-2 space-y-1">
                    <p className="text-red-400 text-[10px] flex items-center gap-2"><AlertCircle size={14} /> {submitError}</p>
                    <a href={`mailto:${emailValue}`} className="text-[#D4AF37] text-[10px] underline hover:text-white block">Pošalji email direktno →</a>
                  </div>
                )}
